# -*- coding: utf-8 -*-
"""MobileNetV2 .h5 -> .onnx via SavedModel intermediate (avoids Keras 3 tensor naming issues)."""
...
logger.info("=" * 60)
logger.info("MobileNetV2 .h5 -> .onnx via SavedModel")
logger.info("=" * 60)
...
try:
    ...
    # Step 2: Export to SavedModel first
    logger.info("Exporting to SavedModel...")
    model.export(str(savedmodel_dir))
    logger.info(f"  SavedModel saved to {savedmodel_dir}")
    
    # Step 3: Convert SavedModel -> ONNX using tf2onnx
    logger.info("Converting SavedModel -> ONNX...")
    import tf2onnx
    
    # Use from_saved_model instead of from_keras
